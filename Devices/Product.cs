    ...
    public partial class Product : Form
    {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                ...
                // Check if there are any records in tbStockIn that reference this product
                cn.Open();
                cm = new SqlCommand("SELECT COUNT(*) FROM tbStockIn WHERE pcode = @pcode", cn);
                cm.Parameters.AddWithValue("@pcode", pcode);
                stockInCount = (int)cm.ExecuteScalar();
                cn.Close();

                if (stockInCount > 0)
                {
                    // Prompt the user to confirm whether they want to delete the related records in tbStockIn
                    DialogResult result = MessageBox.Show("This product has " + stockInCount + " record(s) in tbStockIn. Do you want to delete these records?", "Delete Product", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (result == DialogResult.Yes)
                    {
                        // Delete the related records in tbStockIn
                        cn.Open();
                        cm = new SqlCommand("DELETE FROM tbStockIn WHERE pcode = @pcode", cn);
                        cm.Parameters.AddWithValue("@pcode", pcode);
                        cm.ExecuteNonQuery();
                        cn.Close();
                    }
                    ...
                    {
                        // User cancelled, do not proceed with deleting the product
                        return;
                    }
                }

                // Delete the product from tbProduct
                cn.Open();
